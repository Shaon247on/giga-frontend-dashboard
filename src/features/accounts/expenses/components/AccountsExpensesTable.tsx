...
export function AccountsExpensesTable({
  data,
  totalItems,
  currentPage,
}: AccountsExpensesTableProps) {
  ...
  const handlePreview = (item: AccountExpense) => {
    // Navigate to the preview URL that triggers the parallel route
    router.push(`/accounts/expenses/expense/${item.id}`, { scroll: false });
  };
  ...
  const columns: Column<AccountExpense>[] = [
    {
      key: "employeeName",
      header: "Employee",
      cell: (item) => (
    ...
    },
    {
      ...
      cell: (item) => (
        <span className={cn(
          ...
          item.type === "direct" && "bg-blue-50 text-blue-600",
  ...
  const actions: Action<AccountExpense>[] = [
    {
      label: "Preview",
      icon: <Eye className="w-3.5 h-3.5" />,
      onClick: (item) => handlePreview(item),
      variant: "outline",
      size: "sm",
    },
  ];
