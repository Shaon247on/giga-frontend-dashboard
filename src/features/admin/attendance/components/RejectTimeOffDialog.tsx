...
interface RejectTimeOffDialogProps {
  request: TimeOffRequest | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm: (request: TimeOffRequest, reason: string) => void;
}

export function RejectTimeOffDialog({
  request,
  open,
  onOpenChange,
  onConfirm,
}: RejectTimeOffDialogProps) {
      ...
      <DialogContent className="sm:max-w-105 rounded-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-[13px] text-secondary-txt">
            Please provide a reason for rejecting this time off request for{" "}
